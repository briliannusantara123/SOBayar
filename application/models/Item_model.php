...
public function getDataCD($item_code)
{
	// $array = array('apple', 'banana', 'orange');
	// $array_with_quotes = array_map(function($val) { return "'" . $val . "'"; }, $name);
	// $nama = implode(", ", $array_with_quotes);
	...
	$date = date('Y-m-d');
	$this->db->select('i.*');
	$this->db->where_in('i.no',$item_code);

	return $this->db->get('sh_m_item i')->result();
}
public function getDataQD($item_code)
{
	// $array = array('apple', 'banana', 'orange');
	// $array_with_quotes = array_map(function($val) { return "'" . $val . "'"; }, $name);
	// $nama = implode(", ", $array_with_quotes);
	...
	$date = date('Y-m-d');
	$where ="id_customer ='".$ic."' and id_table ='".$it."' and user_order_id ='".$uoi."' and left(entry_date,10) ='".$date."'";
	$this->db->select('*');
	$this->db->where($where);
	$this->db->where_in('item_code',$item_code);

	return $this->db->get('sh_cart_details')->result();
}
...
public function get_info_item($item_code, $data)
	{
		...
		public function sub_category_paket()
		{
			...
			$this->db->select('*');
	        $this->db->from('sh_m_item_packages');
	        $this->db->where('package_name', $item_code);
		...
		public function getDataPaket($paket,$sub_category)
		{
			...
			$this->db->join('sh_cart c', 'i.no = c.item_code', 'left');
			$this->db->join('sh_t_transaction_details d', 'i.no = d.item_code', 'left');
			$this->db->select('i.sub_category,i.description,i.image_path,i.id,i.harga_weekday,i.harga_weekend,i.harga_holiday,i.no,i.need_stock,i.is_sold_out,c.qty,c.id_customer,i.with_option,i.sub_category,i.stock,d.qty');
			$this->db->where($where);
			$this->db->group_by("i.description");
			$this->db->order_by('i.description asc');
			return $this->db->get('sh_m_item i')->result();
		}
		public function getqtylimit($sub)
		{
			$ic = $this->session->userdata('id');
			$date = date('Y-m-d');
...
//trans
		...
		public function cekcartbayar($ic)
		{
			$date = date('Y-m-d');
			$uoi = $this->session->userdata('user_order_id');
			$this->db->select('m.image_path, m.need_stock, d.*');
			$this->db->from('sh_cart d');
			$this->db->join('sh_m_item m', 'm.no = d.item_code');
			$where = "LEFT(entry_date, 10) = '" . $date . "' AND d.bayar_kasir = 1 AND id_customer = '" . $ic . "' AND user_order_id = '" . $uoi . "'";
			$this->db->where($where);
		...
		public function cartpaket($ic)
		{
			$date = date('Y-m-d');
			$uoi = $this->session->userdata('user_order_id');
			// var_dump($uoi);exit();
			$this->db->select('m.sub_paket_so,m.sub_category,m.image_path,m.need_stock,m.is_sold_out,d.*');
	        $this->db->from('sh_cart d');
	        $this->db->join('sh_m_item m', 'm.no = d.item_code');
	        $where = "left(entry_date,10) ='".$date."' and id_customer = '".$ic."' and user_order_id = '".$uoi."'";
	        $this->db->where($where);
		...
		public function getitempaket($id_detail)
		{
			$date = date('Y-m-d');
			// var_dump($uoi);exit();
			$this->db->select('*');
	        $this->db->from('sh_t_transaction_details_list');
	        $where = "left(created_date,10) ='".$date."' and id_detail = 600";
	        $this->db->where($where);
	              
	        $query = $this->db->get();
	        return $query;
		}
		public function getcartpaket($ic, $paket)
		{
		    $date = date('Y-m-d');
		    $uoi = $this->session->userdata('user_order_id');
		    
		    $this->db->select('d.item_code, m.sub_category, m.image_path, m.need_stock, d.*');
		    $this->db->from('sh_cart_details d');
		    $this->db->join('sh_m_item m', 'm.no = d.item_code');
		    
		    
		    $where = "LEFT(d.entry_date, 10) = '" . $date . "' AND d.id_customer = '" . $ic . "' AND d.user_order_id = '" . $uoi . "'";
		    $this->db->where($where);
		...
		public function cekcartpaket($ic)
		{
			$date = date('Y-m-d');
			$uoi = $this->session->userdata('user_order_id');
			$this->db->select('m.image_path,m.need_stock,d.*');
...
public function bill($cabang,$notrans,$datake)
{
	//get setup
	$q = "select * from sh_m_setup ";
	$setup = $this->db->query($q)->row();
	...
	$taxP = $setup->tax_percent;
	//get setup
	$query = "select c.customer_name, a.id_trans, c.total_pax as totalpax_reservasi, (select count(t.seat_id) as ttl from (select d.seat_id from sh_t_transaction_details d where d.id_trans = ".$notrans." and d.cekdata = ".$datake." group by d.selected_table_no,d.seat_id) as t) as totalpax_actual, (select (sum(d.unit_price * d.qty) - sum(d.unit_price * d.qty * (d.disc/100))) as total from sh_t_transaction_details d where d.is_cancel = 0 and d.id_trans = ".$notrans." and d.cekdata = ".$datake." group by d.id_trans) as total, ((select (sum(d.unit_price * d.qty) - sum(d.unit_price * d.qty * (d.disc/100))) as total from sh_t_transaction_details d where d.is_cancel = 0 and d.id_trans = ".$notrans." and d.cekdata = ".$datake." group by d.id_trans) * (".$scP."/100)) as sc, ((((select (sum(d.unit_price * d.qty) - sum(d.unit_price * d.qty * (d.disc/100))) as total from sh_t_transaction_details d where d.is_cancel = 0 and d.id_trans = ".$notrans." and d.cekdata = ".$datake." group by d.id_trans) * (".$scP."/100)) * (".$taxP."/100)) + ((select (sum(d.unit_price * d.qty) - sum(d.unit_price * d.qty * (d.disc/100))) as total from sh_t_transaction_details d where d.is_cancel = 0 and d.id_trans = ".$notrans." and d.cekdata = ".$datake." group by d.id_trans) * (".$taxP."/100))) as ppn, (select group_concat(xx.id_table) from sh_rel_table xx inner join sh_trans_reltable strx on strx.id_rel_table = xx.id inner join sh_t_transactions tx on tx.id = strx.id_trans where tx.id = ".$notrans.") as no_table, b.bill_printed_count as print_count 
						  from sh_t_transaction_details a inner join sh_t_transactions b on a.id_trans = b.id 
						  inner join sh_m_customer c on c.id = b.id_customer where a.is_paid = 0 and a.is_cancel = 0 and b.cabang = ".$cabang." and b.id= ".$notrans." and Left(b.create_date, 10) = Left(SYSDATE(), 10) limit 1";
	return $this->db->query($query)->row();
}
...
public function totalbayar($notrans)
{
	...
	public function hitungcart($nomeja)
		...
		public function get_sold()
		{
			...
			// $ic = $this->session->userdata('id');
			// // var_dump($uoi);exit();
			// $this->db->select('m.is_sold_out,d.*');
	  //       $this->db->from('sh_cart d');
	  //       $this->db->join('sh_m_item m', 'm.no = d.item_code');
	  //       $where = "left(entry_date,10) ='".$date."' and id_customer = '".$ic."' and user_order_id = '".$uoi."'";
	  //       $this->db->where($where);
	              
	  //       $query = $this->db->get()->row('m.is_sold_out');
			$ic = $this->session->userdata('id');
			$this->db->select('m.*');
	        $this->db->from('sh_cart d');
	        $this->db->join('sh_m_item m', 'm.no = d.item_code');
	        ...
	        $query = $this->db->get()->row('is_sold_out');
	        return $query;
		}
		public function cekbayar($id_trans)
		{
			$sql = "select C.* from sh_cart C inner join sh_rel_table T on C.id_table = T.id_table where C.bayar_kasir = 1 and T.status = 'Payment' and C.id_trans = '".$id_trans."' and left(T.created_date,10) = left(sysdate(),10) limit 1";
