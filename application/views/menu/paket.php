      ...
      <?php if ($i->is_sold_out == 0): ?>
        <div class="container text-center" style="position:relative;">
          ...
          <div class="row" style="margin-top: 5px;">
            <div class="col" >
              <button type="button" class="btn kurang " style="padding-left: 9px;padding-right: 9px;background-color: #223c77;" id="kurang<?= $i->id ?>" onclick="OrderQty('minus','<?= $i->id ?>','<?= $i->no ?>','<?= $i->stock ?>','<?= $i->need_stock ?>','<?= $this->session->userdata('user_order_id') ?>','paket');"> - </button>
            </div>
            <div class="col">
            ...
            <div class="col">
              <?php $options = $this->Item_model->option($i->no); ?>
              <?php if ($i->with_option == 1 || $i->with_option == 2): ?>
                <button type="button" class="btn nambah<?= $i->id ?>" data-bs-toggle="modal" data-bs-target="#exampleModal<?= $i->id ?>" style="padding-left: 7px;padding-right: 7px;background-color: #223c77;">+</button>
              <?php else: ?>
                <button type="button" class="btn tambah" style="padding-left: 7px;padding-right: 7px;background-color: #223c77;" id="tambah<?= $i->id ?>" onclick="OrderQty('plus','<?= $i->id ?>','<?= $i->no ?>','<?= $i->stock ?>','<?= $i->need_stock ?>','<?= $this->session->userdata('user_order_id') ?>','paket');">+</button>
              <?php endif ?>
              </div>
            </div>
          </div>
  ...
  <div class="modal-dialog modal-dialog-scrollable">
    <div class="modal-content">
      ...
      <div class="modal-body">
        <?php if ( $i->image_path != "" ): ?>
        <img src="<?= $i->image_path ?>" alt="Red dot" class="img-fluid" />
        <?php  else: ?>
...
<script type="text/javascript">
  ...
  function OrderQty(tipe,id,no,stock,need_stock,uoi,cek) {
    ...
    var item_code = $('#paket_name').val();
    var subklik = $('#subklik').val();
    console.log(hargapaket);
       if (tipe == 'plus') {
        if (qtyadd.value == qtylimit.value) {
          ...
          $.ajax({
          type:'POST',
          data: {tipe: tipe,id: id,itemcode: itemCode,description: desc,unit_price: price,extra_notes: notes,no: no,stock: stock,need_stock: need_stock,uoi: uoi,notesdua:notesdua,notestiga:notestiga,paket:paket,hargapaket:hargapaket,cek:cek,item_code:item_code,subklik:subklik},
...
<script type="text/javascript">
  ...
  $(document).ready(function(){
    ...
    $('.tambah').click(function() {
      var q = qtyadd.value;
      if (qtyadd.value == qtylimit.value) {
          var isi = "Package Food Menu Exceeds Quantity";
          Swal.fire({
          title: 'Notification!',
          customClass: {
            container :'my-popup-class'
          ...
          localStorage.setItem(subklik.value,qtylimit.value);
      }else{
